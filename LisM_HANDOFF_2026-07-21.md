...
- DYA Studio版では左右共通の `amouse` がMouse Layerの状態と解除タイマーを管理し、解除時間・対象レイヤー・有効/無効を変更できる。
- 左右が別々に同じレイヤーを解除して状態が食い違う競合を避けるため、`rmouse2` / `lmouse2` はポインター倍率・向きだけを管理する。
...
- `amouse`: 左右共通のMouse Layer自動切替。既定は`layer_1`、解除1000ms。
- `rmouse2`: 右ポインターの倍率・向き。既定は1倍、X/Y反転。
- `lmouse2`: 左ポインターの倍率・向き。既定は1倍、軸反転なし。
...
- 旧版のDYA保存設定を引き継いで競合が復活しないよう、ポインター表示名を `rmouse` / `lmouse` から `rmouse2` / `lmouse2` に変更した。旧表示名で変更していた倍率や向きは新表示名で再設定する。
